...
public class StudentServiceTest {
    ...
    public void testImportCSV() {
        ...
        Resource resource = new DefaultResourceLoader().getResource("classpath:data.csv");
        studentService.importCSV(resource);
        Mockito.verify(nodeRepository, Mockito.times(46)).findByName(Mockito.eq("Capital"));
        Mockito.verify(nodeRepository, Mockito.times(4)).findByName(Mockito.eq("Paternal"));
        ...
        Assertions.assertThat(captor.getValue()).hasSize(46);
        ...
        Assertions.assertThat(graduatedCount).isEqualTo(20);
        long count = Arrays.stream(capture.toString().split("\n")).filter(s -> s.endsWith("Node not found Paternal")).count();
        Assertions.assertThat(count).isEqualTo(4L);
    }
}
